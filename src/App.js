...
export default function App() {
  ...
  const [state, setState] = useState({clients:{}, owner:{}, photos:{}, selectedImg: "", selectedClientId: undefined});
  ...
  return (
    <div className="App">
      <Grid container direction='row' justifyContent='space-between' spacing={0}>
        ...
        <Grid item xs={8}>
          <Grid container direction="column" alignItems='center' justifyContent='space-between' spacing={0} className={`${classes.imageListGridContainer} image-list-grid-container`}>
            <Backdrop className={classes.backdrop} open={!(state.selectedClientId)} >
              <Button variant="contained" color="primary" >Select A Client to Start Chat</Button>
            </Backdrop>
            <Grid item>
              <ImageList className={classes.imageList}>
                <ImageListItem key="Subheader" cols={2} style={{ height: 'auto'}} >
                  <ListSubheader component="div">{(state.selectedClientId && state.clients[state.selectedClientId])? `Chatting with ${state.clients[state.selectedClientId].name} - ${state.clients[state.selectedClientId].id}` : ""}</ListSubheader>
                </ImageListItem>
                {state.selectedClientId && state.clients[state.selectedClientId] && Object.values(state.clients[state.selectedClientId].photos).map((item) => (
                  <ImageListItem key={item.id} cols={1}>
                    <img src={item.photoData} alt={item.from} />
                    <ImageListItemBar
                      title={item.id}
                      subtitle={<span>from: {item.isReverted === true ? state.owner.name : state.clients[item.from].name}</span>}
                      actionIcon={
                        <IconButton className={classes.icon} onClick={()=>{deletePhoto(item.id)}}>
                          <DeleteForever />
                        </IconButton>
                      }
                    />
            ...
            <Grid container justifyContent="space-between" alignItems='baseline' spacing={2} className={classes.imageListControlButtons}>
              <Grid item>
